...
def backup_codebase(backup_folder):
    ...
    train_script_path = os.path.join(code_dir, "train.sh")
    
    if os.path.exists(setup_path):
        shutil.copy2(setup_path, backup_folder)
    else:
        print(f"[Warning] {setup_path} 不存在，跳过拷贝。")
    
    if os.path.exists(train_script_path):
        shutil.copy2(train_script_path, backup_folder)
    else:
        print(f"[Warning] {train_script_path} 不存在，跳过拷贝。")